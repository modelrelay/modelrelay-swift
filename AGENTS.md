...
- `ResponsesClient.batch` posts `/responses/batch` with `ResponsesBatchItem` + `ResponsesBatchRequestOptions` (`maxConcurrent`, `failFast`, `itemTimeoutMs`) and returns `ResponsesBatchResponse` usage totals.
